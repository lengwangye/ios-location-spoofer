...
async function refresh(){try{render(await call('/status'))}catch(e){note(e.message,true)}}function placeLabel(p){return[p.name,p.street,p.district,p.city,p.county,p.state,p.country].filter((v,i,a)=>v&&a.indexOf(v)===i).join(' · ')}function setPlace(name){selectedPlaceName=name||'';$('placeName').textContent='当前选点：'+(selectedPlaceName||'尚未识别地点')}async function reversePlace(lat,lng){try{const j=await fetch('https://photon.komoot.io/reverse?lat='+lat+'&lon='+lng).then(r=>r.json()),p=j.features&&j.features[0]&&j.features[0].properties;setPlace(p?placeLabel(p):'坐标 '+Number(lat).toFixed(5)+', '+Number(lng).toFixed(5))}catch(e){setPlace('坐标 '+Number(lat).toFixed(5)+', '+Number(lng).toFixed(5))}}function move(lat,lng,name){$('lat').value=Number(lat).toFixed(7);$('lng').value=Number(lng).toFixed(7);marker&&marker.setLatLng([lat,lng]);map&&map.setView([lat,lng],Math.max(map.getZoom(),15));name?setPlace(name):reversePlace(lat,lng);fetch('https://api.open-meteo.com/v1/elevation?latitude='+lat+'&longitude='+lng).then(r=>r.json()).then(j=>{if(j.elevation&&j.elevation.length)$('alt').value=Math.round(j.elevation[0])}).catch(()=>note('已选点，但自动海拔查询失败',true))}
